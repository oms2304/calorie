...
=======
    ...
    func loadUserGoals(userID: String) {
        db.collection("users").document(userID).getDocument { document, error in
            if let document = document, document.exists, let data = document.data() {
                self.calories = data["calories"] as? Double ?? 2000
                self.protein = data["protein"] as? Double ?? 150
                self.fats = data["fats"] as? Double ?? 70
                self.carbs = data["carbs"] as? Double ?? 250
                self.weight = data["weight"] as? Double ?? 150.0
            } else {
                print("Error fetching user goals: \(error?.localizedDescription ?? "Unknown error")")
>>>>>>> d3d7eb3 (Initial commit)
            }
...
=======
    func saveUserGoals(userID: String) {
        ...
        db.collection("users").document(userID).updateData(["goals": goalData]) { error in
            if let error = error {
                print("Error saving user goals: \(error.localizedDescription)")
            }
        }

...
=======
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            if let currentDailyLog = dailyLogService.currentDailyLog {
                HorizontalBarChartView(dailyLog: currentDailyLog, goal: goalSettings)
                    .padding(.top, 10)
            } else {
                HorizontalBarChartView(dailyLog: placeholderDailyLog(), goal: goalSettings)
                    .padding(.top, 10)
            }

            
            if let currentDailyLog = dailyLogService.currentDailyLog {
                FoodLogView(meals: currentDailyLog.meals)
                    .padding(.top, 10)
            } else {
            ...
            HStack {
                addButton("Add Food Manually", color: .blue) {
                ...
                addButton("Search", color: .green) {
                    showingSearchView = true
                }
                .sheet(isPresented: $showingSearchView) {
                    if let currentLog = dailyLogService.currentDailyLog {
                        USDAFoodSearchView(dailyLog: $dailyLogService.currentDailyLog) { updatedLog in
                            dailyLogService.currentDailyLog = updatedLog
                        }
                    } else {
                ...
                addButton("Scan Barcode", color: .orange) {
                    showingBarcodeScanner = true
                }
                .sheet(isPresented: $showingBarcodeScanner) {
                    BarcodeScannerView { result in
                        scannedProductName = result
                    }
                }
            }
            .padding()
        ...
        if let userID = Auth.auth().currentUser?.uid {
            ...
            dailyLogService.fetchLogs(for: userID) { result in
                ...
                case .success(let logs):
                    if logs.isEmpty {
                        print("No logs found for user: \(userID). Creating initial log.")
                      
                        dailyLogService.createInitialLog(for: userID) { creationResult in
                        ...
                        DispatchQueue.main.async {
                            ...
                            print("Loaded daily log: \(String(describing: dailyLogService.currentDailyLog))")
                        }
                    }
                case .failure(let error):
                    print("Error fetching logs: \(error.localizedDescription)")
    ...
    private func placeholderDailyLog() -> DailyLog {
        return DailyLog(
            id: "placeholder",
            date: Date(),
            meals: [Meal(id: "meal", name: "Placeholder", foodItems: [])],
            totalCaloriesOverride: 0
        )
